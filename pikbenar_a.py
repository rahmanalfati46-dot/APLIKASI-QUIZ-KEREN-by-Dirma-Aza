        ...
        if menu == "Riwayat Nilai":
            ...
            if not skor_data:
                ...
                for i, e in enumerate(skor_data, start=1):
                    st.write(f"{i}. {e['timestamp']} — {e['nama']} — nilai {e['skor']} ({e['benar']}/{e['total']})")

        # ---------------------- RIWAYAT JAWABAN PESERTA -----------------------
        if menu == "Riwayat Jawaban Peserta":
            st.header("🧾 Riwayat Jawaban Peserta")
            if not jawaban_data:
                st.info("Belum ada yang mengerjakan.")
            else:
                for data_user in jawaban_data:
                    st.subheader(f"👤 {data_user['nama']} ({data_user['timestamp']})")
...
if menu == "Grafik Nilai":
    ...
    if not skor_data:
        ...
        if tipe_grafik == "Garis":
            ...
            # Total jawaban benar vs salah dari semua peserta
            total_benar = sum(benar)
            total_salah = sum([t - b for t, b in zip(total, benar)])
            fig, ax = plt.subplots(figsize=(6,6))
            ax.pie([total_benar, total_salah], labels=["Benar", "Salah"], autopct="%1.1f%%", colors=["skyblue", "salmon"])
...
# ======================================================
def mode_terminal():
    print("Streamlit tidak terdeteksi → Mode Terminal aktif.")
    # (Tidak diubah)
